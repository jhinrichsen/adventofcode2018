...
The parser signature depends on the input structure:

* **Line-oriented parsing**: `func NewDayXX(lines []string) (DayXXPuzzle, error)` - when input has distinct line-based structure
* **Byte-level parsing**: `func NewDayXX(data []byte) (DayXXPuzzle, error)` - when parsing coordinates, numbers, or compact formats
* **No recursion**: Use loops for parsing
* **Error handling**: Return errors for invalid input

**Rule of thumb**: If the data is naturally line-oriented (distinct lines with different meanings), use `[]string`. For compact formats requiring byte-level parsing, use `[]byte`.

**Testing helpers available**:

* `linesFromFilename(t, filename)` - returns `[]string` for line-oriented parsers
* `file(t, day)` - returns `[]byte` for byte-level parsers
* `exampleFile(t, day)` - returns `[]byte` for examples

==== Line-Oriented Parsing (Day 3, 4, 6, 7 pattern)

When input has distinct line-based structure (headers, groups, structured lines):
...
func NewDayXX(lines []string) (DayXXPuzzle, error) {
    ...
    for _, line := range lines {
        ...
        if currentSection == "header" {
            group := parseGroupFromLine(line)
            groups = append(groups, group)
        }
    }

    return DayXXPuzzle{groups: groups}, nil
}
----
...
==== Byte-Level Parsing (Day 16, 23, 25 pattern)

When parsing coordinates, numbers, or compact formats:
...
func NewDayXX(data []byte) (DayXXPuzzle, error) {
    ...
    return DayXXPuzzle{items: items}, nil
}
----
...
// Or use testWithParserBytes helper
func TestDayXXPart1(t *testing.T) {
...
The solution function MUST follow this signature:
...
==== For Line-Oriented Parsers (`[]string`)
...
func TestDayXXPart1Example(t *testing.T) {
    lines := linesFromFilename(t, exampleFilename(XX))
    puzzle, err := NewDayXX(lines)
    if err != nil {
        t.Fatal(err)
    }
...
// Using testWithParserBytes helper (recommended)
func TestDayXXPart1Example(t *testing.T) {
    testWithParserBytes(t, XX, exampleFile, true, NewDayXX, DayXX, "expected")
}
...
// OR: Manual approach for table-driven tests
func TestDayXXPart1Examples(t *testing.T) {
    tests := []struct {
        name  string
        input string
        want  string
    }{
        {"example 1", `input data`, "expected1"},
        {"example 2", `input data`, "expected2"},
    }

    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            puzzle, err := NewDayXX([]byte(tt.input))
            if err != nil {
                t.Fatal(err)
            }
            got := DayXX(puzzle, true)
...
* **Platform**: Linux
* **Test helpers**: `testWithParserBytes`, `benchWithParserBytes`, `exampleFile`, `file`
