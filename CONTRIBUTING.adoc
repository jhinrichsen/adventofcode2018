...
* **Signature**: `func NewDayXX(data []byte) (DayXXPuzzle, error)`
* **Input**: Raw `[]byte` (never `string`)
* **Parsing**: Manual index-based iteration (no `strings.Split` for complex parsing)
...
* ❌ Using string operations on []byte
