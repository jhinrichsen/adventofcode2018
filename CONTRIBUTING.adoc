...
**Note**: Use `uint` for numeric answers. Use `string` only for non-numeric answers.
...
----
func TestDayXXPart1(t *testing.T) {
    testWithParserBytes(t, XX, file, true, NewDayXX, DayXX, 12345)
}
...
**Note**: Use `uint` for counts and numeric answers (no cast needed, type is inferred). Use `string` only for non-numeric answers.
...
**Note**: Use `uint` for numeric answers (counts, sums, coordinates). Use `string` only for non-numeric answers.
