...
* **Line-oriented parsing**: `func NewDayXX(data []byte) (DayXXPuzzle, error)` where first operation is `strings.Split(string(data), "\n")`
* **Byte-level parsing**: `func NewDayXX(data []byte) (DayXXPuzzle, error)` with manual index-based iteration
* **No recursion**: Use loops for parsing
* **Error handling**: Return errors for invalid input

**Rule of thumb**: If the first thing you do is split by newlines, the input should already be `[]string`. Otherwise, use `[]byte` for byte-level parsing.

==== Line-Oriented Parsing (Day 24 pattern)

When input has distinct line-based structure (headers, groups, etc.):
...
// Puzzle test (with verified answer)
...
* ❌ Using `strings.Split(string(data), "\n")` as first operation (prefer line-oriented parser design)
